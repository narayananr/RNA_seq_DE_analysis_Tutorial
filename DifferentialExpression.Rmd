...
date: "August 11, 2016, Modified version of RNA-seq workshop at 21st Century Mouse Genetics 2016"
...
Let us create data frames for DESeq2 object with randomized data

```{r random_dataframe_deseq2_10, results="hide"}
### colData contains the condition/group information for Differenetial expression analysis
colData <- DataFrame(group = factor(exp_design_diet_DE$diet))
```
```{r deseq2_obj_10_random, results="hide"}
### Create DESeq2 object using expression and colData
sample(ncol(exp.dietDE))
dds_10reps_rand <- DESeqDataSetFromMatrix(countData = as.data.frame(round(exp.dietDE))[,sample(ncol(exp.dietDE))],
         colData = colData, design = ~ group)
dds_10reps_rand <- DESeq(dds_10reps_rand)
res_10reps_rand = results(dds_10reps_rand)
resOrdered_10reps_rand <- res_10reps_rand[order(res_10reps_rand$padj),]
head(resOrdered_10reps_rand)

```

DE Analysis summary with **10**, and randomized **10** samples per group 
----------------------------------------------------

```{r res_summary_10_rand}
### summary of Differential Expression analysis
summary(res_10reps)
summary(res_10reps_rand)

```


DE Analysis summary with **10**, **3** samples per group 
----------------------------------------------------
...
```{r pval_hist_10_rand, fig.width=12, fig.height=8}
par(mfrow=c(1,2))
hist(res_10reps$pvalue,breaks=100,col="grey", xlab="p-value",main="10 Samples per group")
hist(res_10reps_rand$pvalue,breaks=100,ylim=c(0,1400),col="grey", xlab="p-value",main="10 Random Samples/grp")
```
